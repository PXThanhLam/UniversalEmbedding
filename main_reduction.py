...
Modified from: https://github.com/facebookresearch/xcit
"""
import argparse
import datetime
import numpy as np
...
from engine_ae import train_one_epoch_ae, evaluate_ae
from losses import ReductionAELoss, ReductionAELossWoClassify
import utils

import xcit_retrieval
from clip_model import ClipvitlModel2, ClipvitlModelReduction3
...
def get_args_parser():
    ...
    parser.add_argument('--model', default='xcit_retrievalv2_reduction', type=str, metavar='MODEL',
                        help='Name of model to train')
    ...
    parser.add_argument('--color-jitter', type=float, default=0.25, metavar='PCT',
    ...
    parser.add_argument('--crop_scale', type=float, default=0.75, metavar='PCT',
                        help='min crop scale, defualt = 0.75)')
    parser.add_argument('--crop_p', type=float, default=0.6, metavar='PCT',
    ...
    parser.add_argument('--data-path', default='../split_path', type=str,
                        help='dataset path')
    parser.add_argument('--nb_classes', default= 257184, type=int,
    ...
    parser.add_argument('--val_split', default= 'test_split.txt', type=str,
                        help='Number of class')
    parser.add_argument('--output_dir', default='model_checkpoint/vitl_cip_adapsubarc_tune23highd_d3_reduct_ckpt0_nonlineardecode2_normalize',
    ...
    parser.add_argument("--pretrained_backbone",
                        default='model_checkpoint/ckpt_for_submit_clip_tune23_adapsubarcface_d3_e0.pth', 
...
def main(args):
    ...
    if args.from_timm == True:
        ...
        checkpoint_backbone = torch.load(args.pretrained_backbone, map_location='cpu')
        ...
        checkpoint_backbone = torch.load(args.pretrained_backbone, map_location='cpu')
        checkpoint_backbone_model = checkpoint_backbone
        
        model = ClipvitlModelReduction3(pretrained_backbone = model_backbone )
        model.retrieval_back_bone.load_state_dict(checkpoint_backbone_model, strict=True)
    ...
    if args.resume and os.path.exists(resume_path):
        ...
        utils.freeze_weights(model_without_ddp,['retrieval_back_bone'])
        ######
        model_without_ddp.retrieval_back_bone.load_state_dict(checkpoint_backbone_model, strict=True)
        ######
    ...
    if args.eval:
        test_stats = evaluate_ae(data_loader_val, model, device)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch_ae(
...
if __name__ == '__main__':
    ...
    main(args)
    # checkpoint = torch.load('/work/Lam_working/ImageRetrieval/model_checkpoint/xcit_small_12_p16_retrieval/checkpoint_model_epoch_10.pth', map_location='cpu')
