...
class CurricularFace(nn.Module):
    def __init__(self, in_features, out_features, m = 0.15, s = 30.): # (m = 0.2, s= 30.)
...
class XCiTRetrievalv2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768,
                 ...
                 tokens_norm=False, dilation_rates = [1, 3, 6, 9, 12], reduct = None):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  
        glob = x[:,0]
        glob = self.projector(glob)
        return glob, cls_attn[:,:,1:], x[:,1:], (Hp,Wp)
    ...
    def forward(self, x, label, split = 'train'):
        ...
        global_logit = self.head(global_f,label,split)
