...
from clip_model import ClipvitlModel,ClipvitlModel2,ClipvitlModel3
...
def get_args_parser():
    ...
    parser.add_argument('--cutout_size', type=float, default=0.3, metavar='PCT',
                        help='max cutout relative size (default: 0.3)')
    parser.add_argument('--cutout_p', type=float, default=0.4, metavar='PCT',
                        help='cutout apply prob (default: 0.4)')

    parser.add_argument('--crop_scale', type=float, default=0.7, metavar='PCT',
                        help='min crop scale, defualt = 0.7)')
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='../split_path', type=str,
                        help='dataset path')
    parser.add_argument('--nb_classes', default= 124379, type=int,
                        help='Number of class')
    parser.add_argument('--train_split', default= 'train_split_reduce.txt', type=str,
                        help='Number of class')
    parser.add_argument('--val_split', default= 'test_split_reduce.txt', type=str,
                        help='Number of class')
    parser.add_argument('--output_dir', default='model_checkpoint/xcit_small_12p16_adapsubarc_768d_d3',
    ...
    parser.add_argument("--pretrained", default='model_checkpoint/xcit_small_12_p16_384_dist.pth', type=str, help='Path                                  to pretrained checkpoint')
...
def main(args):
    ...
    if args.from_timm == True:
        ...
        if args.pretrained:
            ...
            print('LOAD PRETRAINED CHECK POINT', args.pretrained )
            model.load_state_dict(checkpoint_model, strict=False)
    else:
        model = ClipvitlModel3(n_class = args.nb_classes)
        free_weight_modules = list(model.state_dict().keys())[:280] #280,294
        utils.freeze_weights(model,free_weight_modules)
...
#         print(name,param.requires_grad)
    ...
    resume_path = os.path.join(output_dir, 'checkpoint_temp_1500_v1.pth') 
    if args.resume and os.path.exists(resume_path):
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            for opt_param in checkpoint_optimizer['param_groups']:
                opt_param['initial_lr'] = args.warmup_lr
            ...
            checkpoint_scheduler['warmup_lr_init'] = args.warmup_lr
            checkpoint_scheduler['warmup_steps'] = [args.warmup_lr,args.warmup_lr]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_grad, output_dir=output_dir,lr_scheduler = lr_scheduler)

        lr_scheduler.step(epoch)
        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint_temp_1500.pth']
...
# python -m torch.distributed.launch --nproc_per_node=2 --use_env main.py --num_workers 3 --batch-size 96 --input-size 336 --from_timm False --output_dir model_checkpoint/vitl_clip_curface_datasetv3_reduce64_keep_22_23 --warmup-lr 7e-5 --lr 1e-4 --resume True --warmup-epochs 1