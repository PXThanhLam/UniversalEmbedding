...
def train_one_epoch(model: torch.nn.Module, criterion: RetrievalLoss ,
                    ...
                    set_training_mode=True, surgery=None, output_dir=None, lr_scheduler = None):
    ...
    for idx, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            global_logit = model(samples,targets)
            loss =criterion(global_logit,targets)
        ...
        ####
        if idx % 3500 == 0:
            output_dir = 'model_checkpoint/vitl_clip_curface_datasetv3_freeze_all'
            ...
            utils.save_on_master({
                ...
                }, os.path.join(output_dir, 'checkpoint_temp_'+ str(idx + 1500) +'.pth'))
        #####
...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 50, header):
        ...
        # compute output
        with torch.cuda.amp.autocast():
            global_logit = model(images,target, split='val')

        loss = criterion(global_logit,target)
