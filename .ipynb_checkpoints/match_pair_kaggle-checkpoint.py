...
def compute_num_inliers(test_keypoints, 
    ...
    try:
        #findHomography,findFundamentalMatrix
        transform_matrix, mask = pydegensac.findHomography(test_match_kp, train_match_kp,
                                            max_reprojection_error,
                                            homography_confidence,
                                            max_ransac_iteration)
    ...
    ###
    print('hormo inlier',int(copy.deepcopy(mask).astype(np.float32).sum()))
    # find hormo than find funda
    inlier_idxs = np.nonzero(inliers)[0]
    test_match_kp, train_match_kp = test_match_kp[inlier_idxs], train_match_kp[inlier_idxs]
    transform_matrix, mask = pydegensac.findFundamentalMatrix(test_match_kp, train_match_kp,
                                            max_reprojection_error,
                                            homography_confidence,
                                            max_ransac_iteration)
    print(transform_matrix)
...
def extract_local_descriptors(model, input, patch_size, attn_threshold, min_attn_threshold, max_key_point, multiscale=None):
    ...
    keep_idx, count = nms(torch.tensor(selected_locations), torch.tensor(all_local_scores), overlap=0.5, top_k=2200)
    keep_idx = keep_idx.cpu().numpy()
    keep_idx = keep_idx[:min(2000,count)]
...
if __name__ == '__main__':
    ...
    # model config
    parser.add_argument('--model', default='xcit_retrievalv2_small_24_p16', type=str, metavar='MODEL',help='')
    parser.add_argument('--patch_size', default=16, type=int, help='Patch resolution of the model.')
    parser.add_argument('--pretrained', default='../model_checkpoint/xcit_small_24_p16_retrieval/checkpoint_model_epoch_29.pth',type=str, help="Path to pretrained weights to load.")
    ...
    parser.add_argument('--location_output_dir', default='', help='')
    ...
    parser.add_argument("--max_distance", default=0.99, type=float)
    ...
    checkpoint_backbone = torch.load(args.pretrained, map_location='cpu')
    ...
    resume_path='../model_checkpoint/xcit_small_24_p16_retrieval_reduction_ae/checkpoint_model_epoch_29.pth'
...
#             print(key,torch.sum(reduce-orig))
    ...
    gt_for_querry_image_paths =  np.concatenate([cfg['gnd'][quer_idx]['hard']]) 
    ...
    #
    all_paths = ['../image-matching-challenge-data/test_images/d91db836/81dd07fb7b9a4e01996cee637f91ca1a.png',
                 '../image-matching-challenge-data/test_images/d91db836/0006b1337a0347f49b4e651c035dfa0e.png']
#     ['../image-matching-challenge-data/test_images/d91db836/81dd07fb7b9a4e01996cee637f91ca1a.png',
#      '../image-matching-challenge-data/test_images/d91db836/0006b1337a0347f49b4e651c035dfa0e.png']
#     ['../image-matching-challenge-data/test_images/6ceaefff/39563e58b2b7411da3f06427c9ee4239.png',
#      '../image-matching-challenge-data/test_images/6ceaefff/0303b05ca0cb46959eac430e4b2472ca.png']
#     ['../image-matching-challenge-data/test_images/1cf87530/0143f47ee9e54243a1b8454f3e91621a.png',
#      '../image-matching-challenge-data/test_images/1cf87530/a5a9975574c94ff9a285f58c39b53d2c.png']
    #
    for im_idx,im_path in tqdm(enumerate(all_paths)):
        ...
        if args.location_output_dir != '' and args.descriptor_output_dir != '' :
            location_path = im_path.replace('../','').replace('/', '__') + '.npy'
        ...
        globals_desc = model.get_global_feat(img_input.to(device))[0].cpu().data.numpy()
        ...
        pair_globs.append(globals_desc)
